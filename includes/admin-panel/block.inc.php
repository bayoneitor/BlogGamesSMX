...
if (isset($_POST['ban-submit'])) {
    ...
    if (!mysqli_stmt_prepare($stmt, $sql)) {
        ...
        if ($resultCheck > 0) {
            $query = "UPDATE users SET Blocked=1, BlockedReason='$cause' WHERE uidUsers='$username'";

            if (mysqli_query($conn, $query)) {
                header("Location: ../../admin-panel/select/block.php?userbanned=success");
             }
        } else {
            header("Location: ../../admin-panel/select/block.php?error=usernotexist");
            exit();
        }
    }
} else { 
