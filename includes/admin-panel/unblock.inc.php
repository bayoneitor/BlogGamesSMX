...
if (isset($_POST['unban-submit'])) {
    ...
    if (!mysqli_stmt_prepare($stmt, $sql)) {
        header("Location: ../../admin-panel/select/block.php?rerror=sqlerror");
        exit();
    } else {
        ...
        if ($resultCheck > 0) {
            $query = "UPDATE users SET Blocked=0, BlockedReason='' WHERE uidUsers='$username'";

            if (mysqli_query($conn, $query)) {
                header("Location: ../../admin-panel/select/unblock.php?userunblocked=success");
            }
        } else {
            header("Location: ../../admin-panel/select/unblock.php?error=usernotexist");
            exit();
        }
    }
} else {
